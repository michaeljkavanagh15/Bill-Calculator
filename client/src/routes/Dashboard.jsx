...
export default function Dashboard() {
  ...
  function deleteItem(type, item) {
    if (type === "Bills") {
      const new_items = Object.fromEntries(
        Object.entries(billsData).filter(([key]) => key.includes(item))
      );
      setBillsData(new_items);
    } else if (type === "Savings") {
      const new_items = Object.fromEntries(
        Object.entries(savingsData).filter(([key]) => key.includes(item))
      );
      setSavingsData(new_items);
    } else if (type === "Income") {
      const new_items = Object.fromEntries(
        Object.entries(incomeData).filter(([key]) => key.includes(item))
      );
      setIncomeData(new_items);
    }
  }
